...
export default class BoardsController {
  ...
  async createBoard(
    ...
    const board = this.createBoardApp.create(boardData, userId);
  ...
  @UseGuards(JwtAuthenticationGuard)
  @Delete(':boardId')
  deleteBoard(@Param() params: BaseParam, @Req() request: RequestWithUser) {
    const {
      user: { _id: userId },
    } = request;
    const { boardId } = params;
    const removed = this.deleteBoardApp.delete(boardId, userId);
    if (!removed) throw new BadRequestException(DELETE_FAILED);
    return removed;
  }
}
